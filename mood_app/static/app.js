...
function renderAuth(app) { var mode="login";
    function h() { return '<div class="auth-container"><div class="auth-card"><div class="logo">🌈</div><h2>'+(mode==="login"?"欢迎回来":"创建账号")+'</h2><p class="subtitle">'+(mode==="login"?"登录心情日记":"加入心情日记社区")+'</p><div class="form-group"><label>用户名</label><input type="text" id="auth-un" placeholder="请输入用户名" maxlength="20" autocomplete="username"></div><div class="form-group"><label>密码</label><div class="pw-wrap"><input type="password" id="auth-pw" placeholder="请输入密码" autocomplete="'+(mode==="login"?"current-password":"new-password")+'">'+makePwToggle("auth-pw")+'</div></div><button class="btn btn-primary btn-lg" id="auth-btn">'+(mode==="login"?"登 录":"注 册")+'</button><div class="auth-switch">'+(mode==="login"?"还没有账号？":"已有账号？")+' <a id="auth-sw">'+(mode==="login"?"立即注册":"立即登录")+'</a></div></div></div>'; }
    app.innerHTML = h();
    ...
    async function submit() { var u = $el("auth-un").value.trim(), p = $el("auth-pw").value; if(!u||!p) { toast("请填写用户名和密码",true); return; } if(u.length<2) { toast("用户名至少2个字符",true); return; } if(p.length<4) { toast("密码至少4个字符",true); return; } var btn = $el("auth-btn"); btn.disabled = true; btn.textContent = "处理中..."; try { var res = mode==="login" ? await API.login(u,p) : await API.register(u,p); API.token = res.token; API.user = res.user; localStorage.setItem("token", res.token); localStorage.setItem("user", JSON.stringify(res.user)); applyTheme(API.user.theme); toast(res.message||"成功！"); updateSidebar(); navigate("feed"); } catch(e) { toast(e.message,true); } btn.disabled = false; btn.textContent = mode==="login"?"登 录":"注 册"; }
}
