...
var API = {
    ...
    request: async function(method, url, body, isForm) {
    ...
    updateProfile: function(data) { return this.request("PUT", "/api/profile", data); },
...
function toast(msg, isErr) {
    var el = getEl("toast"); if (!el) return;
    el.textContent = msg;
...
function fmtDate(iso) {
    var d = new Date(iso);
    return d.getFullYear() + "年" + (d.getMonth() + 1) + "月" + d.getDate() + "日加入";
}
function esc(s) {
    if (!s) return "";
...
function getAvatarColor(uid) {
    var idx = 0;
    if (typeof uid === "string") { for (var i = 0; i < uid.length; i++) idx += uid.charCodeAt(i); }
    else idx = uid || 0;
...
function userAvatarHTML(user) {
    var emoji = user.avatar_emoji || user.username[0].toUpperCase();
    var color = user.avatar_color || getAvatarColor(user.username);
    return '<div class="avatar avatar-' + color + '">' + esc(emoji) + '</div>';
}

function userAvatarLargeHTML(user) {
    var emoji = user.avatar_emoji || user.username[0].toUpperCase();
    var color = user.avatar_color || getAvatarColor(user.username);
    return '<div class="profile-avatar-lg avatar-' + color + '">' + esc(emoji) + '</div>';
}

function makePwToggle(inputId) {
    return '<span class="pw-toggle" onclick="togglePw(\'' + inputId + '\')" title="显示密码">👁️</span>';
}

function togglePw(id) {
    var el = getEl(id);
    if (!el) return;
    if (el.type === "password") { el.type = "text"; el.nextElementSibling.textContent = "🙈"; }
    else { el.type = "password"; el.nextElementSibling.textContent = "👁️"; }
}
    ...
    if (API.token && API.user) {
        ...
        for (var i = 0; i < links.length; i++) {
            ...
            a.onclick = (function(view, uid) {
                return function(e) { e.preventDefault(); navigate(view, { userId: uid }); };
            })(links[i][0], links[i][3]);
            nav.appendChild(a);
        }

        var emoji = API.user.avatar_emoji || API.user.username[0].toUpperCase();
        var color = API.user.avatar_color || getAvatarColor(API.user.username);
        userDiv.innerHTML =
            '<div class="avatar avatar-' + color + '" style="width:40px;height:40px;font-size:16px">' + esc(emoji) + '</div>' +
        ...
        var lb = getEl("btn-logout");
        if (lb) lb.onclick = function(e) { e.stopPropagation(); logout(); };
    } else {
...
function navigate(view, data) {
    ...
    var app = getEl("app"), rp = getEl("right-panel");
    if (!app) return;
    ...
    inp.oninput = function() {
        ...
        timer = setTimeout(async function() {
            try {
                ...
                var el = getEl("search-results"); if (!el) return;
                if (users.length === 0) {
                    ...
                    for (var i = 0; i < users.length; i++) {
                        var u = users[i];
                        var emoji = u.avatar_emoji || u.username[0].toUpperCase();
                        var color = u.avatar_color || getAvatarColor(u.username);
                        h += '<div class="user-row" onclick="navigate(\'profile\',{userId:' + u.id + '})">' +
                            '<div class="avatar-sm avatar-' + color + '">' + esc(emoji) + '</div>' +
...
async function loadRightStats() {
    var el = getEl("right-stats"); if (!el) return;
    try {
        var d = await API.getStats();
        var total = 0, keys = Object.keys(d.mood_distribution);
        ...
        var h = "", entries = Object.entries(d.mood_distribution);
        for (var i = 0; i < entries.length; i++) {
            var k = entries[i][0], v = entries[i][1];
            h += '<div class="stat-row"><span class="stat-emoji">' + MOODS[k].emoji + '</span>' +
                '<div class="bar-mini"><div class="bar-mini-fill" style="width:' + ((v/total)*100).toFixed(1) + '%;background:' + cols[k] + '"></div></div>' +
...
function renderAuth(app) {
    ...
    function html() {
        return '<div class="auth-container"><div class="auth-card">' +
            ...
            '<input type="text" id="auth-un" placeholder="请输入用户名" maxlength="20"></div>' +
            '<div class="form-group"><label>密码</label>' +
            '<div class="pw-wrap"><input type="password" id="auth-pw" placeholder="请输入密码">' + makePwToggle("auth-pw") + '</div></div>' +
            '<button class="btn btn-primary btn-lg" id="auth-btn">' + (mode==="login"?"登 录":"注 册") + '</button>' +
            '<div class="auth-switch">' + (mode==="login"?"还没有账号？":"已有账号？") +
            ' <a id="auth-sw">' + (mode==="login"?"立即注册":"立即登录") + '</a></div></div></div>';
    }
    app.innerHTML = html();
...
function moodCard(r, showDel) {
    ...
    // Use color from username hash for consistency
    var avaColor = getAvatarColor(r.username);
    return '<div class="mood-card">' +
        '<div class="mood-card-header">' +
        '<div class="avatar avatar-' + avaColor + '" onclick="event.stopPropagation();navigate(\'profile\',{userId:' + r.user_id + '})">' + esc((r.username||"?")[0].toUpperCase()) + '</div>' +
...
function renderFeed(app) {
    ...
    var color = API.user ? (API.user.avatar_color || getAvatarColor(API.user.username)) : "purple";
    var emoji = API.user ? (API.user.avatar_emoji || API.user.username[0].toUpperCase()) : "?";

    var moodOpts = "", keys = Object.keys(MOODS);
    for (var i = 0; i < keys.length; i++) {
        var k = keys[i];
        moodOpts += '<span class="mood-option' + (k==="happy"?" selected":"") + '" data-mood="' + k + '">' + MOODS[k].emoji + ' ' + MOODS[k].name + '</span>';
    }
    ...
    app.innerHTML =
        ...
        '<div class="avatar avatar-' + color + '">' + esc(emoji) + '</div>' +
    ...
    for (var i = 0; i < moodEls.length; i++) {
        moodEls[i].onclick = function() {
            selMood = this.getAttribute("data-mood");
    ...
    if (ta) ta.oninput = function() { var c = getEl("char-cnt"); if (c) c.textContent = this.value.length; };

    var imgFile = null;
    var btnImg = getEl("btn-img"), imgInput = getEl("img-input");
    if (btnImg && imgInput) {
        btnImg.onclick = function() { imgInput.click(); };
        imgInput.onchange = function() {
            imgFile = this.files[0]; var pv = getEl("img-preview"); if (!pv) return;
            if (imgFile) {
                ...
                var rm = getEl("rm-img"); if (rm) rm.onclick = function() { imgFile = null; pv.innerHTML = ""; imgInput.value = ""; };
            }
        };
    }

    var btnSub = getEl("btn-sub");
    if (btnSub) btnSub.onclick = async function() {
        var ta = getEl("mood-textarea"); if (!ta) return;
        var content = ta.value.trim();
        if (!content) { toast("请写下心情描述", true); return; }
        var fd = new FormData();
        fd.append("mood", selMood); fd.append("content", content); fd.append("tags", "");
        if (imgFile) fd.append("image", imgFile);
        btnSub.disabled = true; btnSub.textContent = "发布中...";
        try {
            await API.createMood(fd);
            toast("✨ 发布成功！");
            ta.value = ""; var c = getEl("char-cnt"); if (c) c.textContent = "0";
            var pv = getEl("img-preview"); if (pv) pv.innerHTML = "";
            if (imgInput) imgInput.value = ""; imgFile = null;
            feedPage = 1; loadFeed();
        } catch(e) { toast(e.message, true); }
    ...
    for (var i = 0; i < tabs.length; i++) {
        tabs[i].onclick = function() {
            ...
            feedMode = this.getAttribute("data-mode"); feedPage = 1; feedMore = true; loadFeed();
        };
    }
    ...
    window.onscroll = function() {
        ...
        if (document.documentElement.scrollHeight - window.scrollY - window.innerHeight < 300) { feedPage++; loadFeed(true); }
    };
}

async function loadFeed(append) {
    if (feedBusy) return; feedBusy = true;
    var list = getEl("feed-list"), loader = getEl("feed-loader");
    ...
    try {
        ...
        for (var i = 0; i < data.records.length; i++) {
            h += moodCard(data.records[i], data.records[i].user_id === (API.user?API.user.id:-1));
        }
        if (append) list.insertAdjacentHTML("beforeend", h);
        else list.innerHTML = data.records.length===0
            ? '<div class="empty-state"><div class="icon">📝</div><p>' + (feedMode==="friends"?"关注好友后查看":"还没有心情记录，快来写第一条吧") + '</p></div>'
            : h;
    ...
    } catch(e) { if (!append) list.innerHTML = '<div class="empty-state"><p>加载失败：' + esc(e.message) + '</p></div>'; }
    if (loader) loader.style.display = "none"; feedBusy = false;
}

/* ==================== Friends ==================== */
function renderFriends(app) {
    app.innerHTML = '<div class="page-header">👥 好友</div><div id="frd-ctn"><div class="spinner"></div></div>'; loadFriends();
}
async function loadFriends() {
    var el = getEl("frd-ctn"); if (!el) return;
    try {
        var reqs = await API.getFriendRequests(), frds = await API.getFriends(), h = "";
        if (reqs.length) {
            ...
            for (var i = 0; i < reqs.length; i++) {
                var r = reqs[i], col = getAvatarColor(r.sender_name);
                h += '<div class="friend-request-item">' +
                    '<div class="avatar avatar-' + col + '" style="width:40px;height:40px;font-size:16px;cursor:pointer" onclick="navigate(\'profile\',{userId:' + r.sender_id + '})">' + esc(r.sender_name[0].toUpperCase()) + '</div>' +
                    ...
                    '<div class="friend-actions"><button class="btn btn-primary btn-sm" onclick="event.stopPropagation();handleFR(' + r.id + ',\'accept\')">接受</button>' +
        ...
        else for (var i = 0; i < frds.length; i++) {
            var f = frds[i], col = f.avatar_color || getAvatarColor(f.username), em = f.avatar_emoji || f.username[0].toUpperCase();
            h += '<div class="friend-item">' +
                '<div class="avatar avatar-' + col + '" style="width:40px;height:40px;font-size:16px;cursor:pointer" onclick="navigate(\'profile\',{userId:' + f.id + '})">' + esc(em) + '</div>' +
...
function renderFamilies(app) {
    ...
    var cf = getEl("btn-cf"); if (cf) cf.onclick = showCreateFam;
    var jf = getEl("btn-jf"); if (jf) jf.onclick = showJoinFam;
    loadFamilies();
}
async function loadFamilies() {
    var el = getEl("fam-list"); if (!el) return;
    try {
        var fs = await API.getMyFamilies();
        el.innerHTML = fs.length===0 ? '<div class="empty-state"><div class="icon">👨‍👩‍👧</div><p>还没有加入家庭组</p></div>'
            : (function() { var h = ""; for (var i = 0; i < fs.length; i++) { var f = fs[i];
...
function showCreateFam() {
    showModal("创建家庭组",
        '<div class="form-group"><label>名称</label><input type="text" id="fam-name" placeholder="起个名字" maxlength="80"></div>' +
        '<div class="form-group"><label>描述</label><input type="text" id="fam-desc" placeholder="简单介绍" maxlength="200"></div>' +
        '<button class="btn btn-primary" style="width:100%;justify-content:center" id="btn-docf">创建</button>');
    var b = getEl("btn-docf"); if (b) b.onclick = async function() {
        var n = getEl("fam-name"); if (!n) return; var name = n.value.trim(); if (!name) { toast("请输入名称", true); return; }
        try { await API.createFamily(name, getEl("fam-desc")?getEl("fam-desc").value.trim():""); toast("创建成功！"); closeModal(); loadFamilies(); }
        catch(e) { toast(e.message, true); }
    };
}
function showJoinFam() {
    showModal("加入家庭组",
        '<div class="form-group"><label>邀请码</label><input type="text" id="inv-code" placeholder="6位邀请码" maxlength="6"></div>' +
        '<button class="btn btn-primary" style="width:100%;justify-content:center" id="btn-doj">加入</button>');
    var b = getEl("btn-doj"); if (b) b.onclick = async function() {
        var c = getEl("inv-code"); if (!c) return; var code = c.value.trim(); if (!code) { toast("请输入邀请码", true); return; }
        try { var r = await API.joinFamily(code); toast(r.message); closeModal(); loadFamilies(); }
        catch(e) { toast(e.message, true); }
    };
}
...
async function loadFamilyDetail(gid) {
    ...
    try {
        var g = await API.getFamilyDetail(gid), md = await API.getFamilyMoods(gid, 1);
        var chips = "";
        for (var i = 0; i < g.members.length; i++) {
            var m = g.members[i], col = getAvatarColor(m.username);
            chips += '<span class="member-chip" onclick="navigate(\'profile\',{userId:' + m.user_id + '})"><span class="chip-avatar avatar-' + col + '">' + esc(m.username[0].toUpperCase()) + '</span>' + esc(m.username) + '</span>';
        }
        var mh = md.records.length===0 ? '<div class="empty-state"><p>还没有心情记录</p></div>' : "";
        for (var i = 0; i < md.records.length; i++) mh += moodCard(md.records[i], md.records[i].user_id === (API.user?API.user.id:-1));
        app.innerHTML =
            ...
            '<div style="font-size:13px;font-weight:650;margin-bottom:6px">成员 (' + g.members.length + ')</div><div class="member-chips">' + chips + '</div>' +
            '<div style="margin-top:14px;display:flex;gap:8px">' +
            (!g.is_creator&&g.is_member?'<button class="btn btn-outline btn-sm" id="btn-leave">退出家庭组</button>':"") +
            (g.is_creator?'<button class="btn btn-danger btn-sm" id="btn-disband">解散家庭组</button>':"") + '</div></div>' +
            '<div id="fam-moods">' + mh + '</div>';
        ...
        if (db) db.onclick = async function() { if (!confirm("确定解散？不可撤销！")) return; try { await API.disbandFamily(gid); toast("已解散"); navigate("families"); } catch(e) { toast(e.message, true); } };
    } catch(e) { app.innerHTML = '<div class="empty-state"><p>加载失败</p></div>'; }
}

/* ==================== Profile ==================== */
function renderProfile(app, userId) {
    var selfId = API.user ? API.user.id : null, eid = userId || selfId, isSelf = selfId && String(selfId) === String(eid);
    app.innerHTML = '<div class="page"><div class="spinner"></div></div>'; loadProfile(eid, isSelf);
}
async function loadProfile(uid, isSelf) {
    var app = getEl("app"); if (!app) return;
    try {
        var p = await API.getProfile(uid), md = await API.getUserMoods(uid, 1);
        var col = p.avatar_color || getAvatarColor(p.username), em = p.avatar_emoji || p.username[0].toUpperCase();
        var mh = "";
        if (md.records.length===0) mh = '<div class="empty-state"><div class="icon">📭</div><p>还没有心情记录</p></div>';
        else for (var i = 0; i < md.records.length; i++) mh += moodCard(md.records[i], String(md.records[i].user_id) === String(API.user?API.user.id:-1));
        ...
        app.innerHTML =
            ...
            '<div class="profile-avatar-lg avatar-' + col + '">' + esc(em) + '</div>' +
            '<div class="profile-name">' + esc(p.username) + '</div>' +
            '<div class="profile-handle">@' + esc(p.username) + ' · ' + fmtDate(p.created_at) + '</div>' +
            ...
            (isSelf
                ? '<button class="btn btn-outline btn-sm" id="btn-edit">✏️ 编辑资料</button> <button class="btn btn-outline btn-sm" id="btn-settings">⚙️ 账号设置</button>'
                : '<button class="btn btn-primary btn-sm" id="btn-friend">➕ 添加好友</button>') +
            '</div></div><div id="profile-moods">' + mh + '</div>';
        if (isSelf) {
            var eb = getEl("btn-edit"); if (eb) eb.onclick = function() { showEditProfile(p); };
            var sb = getEl("btn-settings"); if (sb) sb.onclick = function() { navigate("settings"); };
        } else {
            var fb = getEl("btn-friend"); if (fb) fb.onclick = async function() {
...
function showEditProfile(p) {
    ...
    for (var i = 0; i < AVATAR_COLORS.length; i++) {
        ...
        colorOpts += '<span class="color-option avatar-' + c + (p.avatar_color===c?" selected":"") + '" data-color="' + c + '" onclick="selectAvatarColor(this,\'' + c + '\')"></span>';
    }
    var emojiOpts = "";
    for (var i = 0; i < AVATAR_EMOJIS.length; i++) {
        var em = AVATAR_EMOJIS[i];
        emojiOpts += '<span class="emoji-option' + (p.avatar_emoji===em?" selected":"") + '" data-emoji="' + em + '" onclick="selectAvatarEmoji(this,\'' + em + '\')">' + em + '</span>';
    }

    showModal("编辑资料",
        '<div class="form-group"><label>用户名</label><input type="text" id="edit-username" value="' + esc(p.username||"") + '" maxlength="20"></div>' +
        '<div class="form-group"><label>简介</label><textarea id="edit-bio" maxlength="200" placeholder="介绍一下自己...">' + esc(p.bio||"") + '</textarea></div>' +
        '<div class="form-group"><label>头像颜色</label><div class="color-picker" id="color-picker">' + colorOpts + '</div></div>' +
        '<div class="form-group"><label>头像表情</label><div class="emoji-picker" id="emoji-picker">' + emojiOpts + '</div></div>' +
        '<button class="btn btn-primary" style="width:100%;justify-content:center;margin-top:8px" id="btn-save-profile">保存</button>'
    );

    var selColor = p.avatar_color || "purple";
    var selEmoji = p.avatar_emoji || "";

    window.selectAvatarColor = function(el, color) {
        selColor = color;
        var all = document.querySelectorAll("#color-picker .color-option");
        for (var i = 0; i < all.length; i++) all[i].classList.remove("selected");
        el.classList.add("selected");
    };
    window.selectAvatarEmoji = function(el, emoji) {
        selEmoji = emoji;
        var all = document.querySelectorAll("#emoji-picker .emoji-option");
        for (var i = 0; i < all.length; i++) all[i].classList.remove("selected");
        el.classList.add("selected");
    };

    var saveBtn = getEl("btn-save-profile");
    if (saveBtn) saveBtn.onclick = async function() {
        var un = getEl("edit-username"); var bio = getEl("edit-bio");
        var data = {};
        if (un) { var newUn = un.value.trim(); if (newUn !== p.username) data.username = newUn; }
        if (bio) { var newBio = bio.value.trim(); if (newBio !== (p.bio||"")) data.bio = newBio; }
        if (selColor !== p.avatar_color) data.avatar_color = selColor;
        if (selEmoji !== (p.avatar_emoji||"")) data.avatar_emoji = selEmoji;

        if (Object.keys(data).length === 0) { toast("没有修改任何内容"); return; }
...
function renderSettings(app) {
    app.innerHTML = '<div class="page">' +
        ...
        '<div class="pw-wrap"><input type="password" id="s-old-pw" placeholder="输入原密码">' + makePwToggle("s-old-pw") + '</div></div>' +
        '<div class="form-group"><label>新密码</label>' +
        '<div class="pw-wrap"><input type="password" id="s-new-pw" placeholder="输入新密码（至少4位）">' + makePwToggle("s-new-pw") + '</div></div>' +
        '<button class="btn btn-primary" id="btn-chpw">修改密码</button></div></div>';
    var b = getEl("btn-chpw"); if (!b) return;
    b.onclick = async function() {
        var o = getEl("s-old-pw"), n = getEl("s-new-pw"); if (!o || !n) return;
        var op = o.value, np = n.value;
        if (!op || !np) { toast("请填写完整", true); return; }
        if (np.length < 4) { toast("新密码至少4个字符", true); return; }
        try { await API.changePassword(op, np); toast("密码修改成功！"); navigate("profile", { userId: API.user?API.user.id:null }); }
        catch(e) { toast(e.message, true); }
    };
}
...
async function loadStats() {
    var app = getEl("app"); if (!app) return;
    try {
        var d = await API.getStats(), total = 0, keys = Object.keys(d.mood_distribution);
        for (var i = 0; i < keys.length; i++) total += d.mood_distribution[keys[i]];
        if (total===0) total = 1;
        var cols = { happy:"#f59e0b", calm:"#6366f1", sad:"#3b82f6", anxious:"#ef4444", excited:"#ec4899", tired:"#8b5cf6" };
        var distH = "", entries = Object.entries(d.mood_distribution);
        for (var i = 0; i < entries.length; i++) {
            var k = entries[i][0], v = entries[i][1];
        ...
        var medals = ["🥇","🥈","🥉"], topH = d.top_users.length===0 ? '<p style="color:var(--text-secondary);text-align:center;padding:12px">暂无数据</p>' : "";
        for (var i = 0; i < d.top_users.length; i++) {
            var u = d.top_users[i];
            topH += '<div class="friend-item"><span style="font-size:22px;width:30px">' + (medals[i]||(i+1)) + '</span>' +
                '<span style="font-weight:650;flex:1">' + esc(u.username) + '</span><span style="color:var(--text-secondary)">' + u.count + ' 条</span></div>';
        }
        app.innerHTML = '<div class="page-header">📊 统计</div>' +
...
function showModal(title, body) {
    var content = getEl("modal-content"); if (!content) return;
    ...
    var ov = getEl("modal-overlay"); if (ov) { ov.style.display = "flex"; ov.onclick = function(e) { if (e.target===ov) closeModal(); }; }
}
function closeModal() { var ov = getEl("modal-overlay"); if (ov) ov.style.display = "none"; }
function openImg(url) { var vi = getEl("image-viewer"), img = getEl("image-viewer-img"); if (vi&&img) { img.src = url; vi.style.display = "flex"; vi.onclick = closeImg; } }
function closeImg() { var vi = getEl("image-viewer"); if (vi) vi.style.display = "none"; }

/* ==================== Logout ==================== */
function logout() {
    API.token = null; API.user = null;
    localStorage.removeItem("token"); localStorage.removeItem("user");
    updateSidebar(); navigate("login");
}
