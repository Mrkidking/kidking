...
class User(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id, "username": self.username, "email": self.email or "", "bio": self.bio,
...
class MoodLike(db.Model):
    __tablename__ = "mood_like"
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    mood_id = db.Column(db.Integer, db.ForeignKey("mood_record.id"), nullable=False)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
...
class Comment(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id, "user_id": self.user_id,
            "username": self.user.username,
            "mood_id": self.mood_id, "content": self.content,
